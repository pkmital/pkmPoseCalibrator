...
pkmPoseCalibrator::pkmPoseCalibrator()
{
	...
	numViews				= 20;				// observations for each calibration point
	numPoints				= 6;				// number of eigen values to train on
...
{		
	...
	Mat rc = r.colRange(0, numPoints);
	values.colRange(0, numPoints).copyTo(rc);
	r.at<double>(0,numPoints) = 1;
...
{
	if (bBuiltModel) {
		Mat px = values.colRange(0, numFactors) * poseXInv;
		px.copyTo(pose_x);
		Mat py = values.colRange(0, numFactors) * poseYInv;
		py.copyTo(pose_y);
